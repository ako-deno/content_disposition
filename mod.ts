/*!
 * Based on https://github.com/jshttp/content-disposition/blob/master/index.js
...
export function contentDisposition(filename?: string, opts: ContentDispositionOptions = {}): string {
  ...
  let header = opts.type ? opts.type.toLowerCase() : 'attachment';

  // append parameters
  if (params) {
    for (const key of Object.keys(params).sort()) {
      const param = params[key];
      const value = key.endsWith('*') ? ustring(param) : qstring(param);
...
function createParams(filename?: string, fallback?: string): Record<string, string>|null {
  ...
  const fallbackName = !fallback ? getlatin1(name) : basename(fallback);
...
function decodeField(str: string): string {
  ...
  // Decode pecent escaped strings to binary string
  const binary = encoded.replace(HEX_ESCAPE_REPLACE_REGEXP, (_, hex) => {
 ...
 */
export function qstring(value: string): string {
